...
### 3.2.3. Weed density ((a) rotation mean density and (b) proportion h & vh in year 6)
```{r, attr.source='.numberLines'}
rm(density_rot)
...
sources_of_costs <- dplyr::left_join(mean_cost_diff_df, min_cost_diff_df, by = c("region", "soiltype", "initDR", "source_of_cost_difference")) %>% 
  ...
  dplyr::mutate(across('source_of_cost_difference', ~ str_replace(., 'cyl', 'lost yield costs'))) %>%
  dplyr::mutate(across('source_of_cost_difference', ~ str_replace(., 'herb', 'herbicide costs'))) %>%
  dplyr::mutate(across('source_of_cost_difference', ~ str_replace(., 'ops', 'operations costs'))) %>%
  dplyr::mutate(across('source_of_cost_difference', ~ str_replace(., 'fert', 'fertiliser costs'))) %>% 
  dplyr::mutate(across('source_of_cost_difference', ~ str_replace(., 'seed', 'seed costs'))) %>% 
  dplyr::mutate(across('source_of_cost_difference', ~ str_replace(., 'sundry', 'sundry costs'))) %>% 
...
pdf("../figures/costs_and_output_1col", height = 30, width = 10)
print(CO_1col)
dev.off()
```
