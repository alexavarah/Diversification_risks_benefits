...
(a) WRONG This gives min and max values across regions and soil types. Used in the paper.  
```{r}
c_gp_yld %>%
...
dplyr::filter(scenario == "MIT") %>% 
  ...
  # 1. Get mean, min and max differences for each strategy
  dplyr::group_by(scenario, region, soiltype, initDR) %>% 
  dplyr::mutate(
    ghg_strategy_diff_mean = mean(c_emiss_diff),
    ghg_strategy_diff_min = min(c_emiss_diff), # this is the min value of any year for a particular strategy.
    ghg_strategy_diff_max = max(c_emiss_diff)
  ) %>%
  ungroup() %>%
  select(scenario, region, soiltype, initDR, ghg_strategy_diff_mean, ghg_strategy_diff_min, ghg_strategy_diff_max) %>%
  dplyr::distinct(scenario, region, soiltype, initDR, .keep_all = TRUE) %>% 
    
  # 2. Calculate mean, min, max for each level of initDR
  dplyr::group_by(initDR) %>% 
  dplyr::mutate(
    ghg_initDR_diff_mean = mean(ghg_strategy_diff_mean),
    ghg_initDR_diff_min_mean = mean(ghg_strategy_diff_min),
    ghg_initDR_diff_max_mean = mean(ghg_strategy_diff_max),
    overall_min = min(ghg_strategy_diff_min),
    overall_max = max(ghg_strategy_diff_max)
  ) %>% 
  
  # 4. Select just useful columns and filter out duplicates
  dplyr::select(initDR, 
                ghg_initDR_diff_mean, ghg_initDR_diff_min_mean, ghg_initDR_diff_max_mean, overall_min, overall_max) %>% 
  dplyr::distinct(initDR, .keep_all = TRUE) %>% 
  arrange(initDR)
```
...
pdf("../figures/emissions_sources_1col_new.pdf", height = 30, width = 10)
print(sources_1col)
dev.off()
```
...
```{r}
# north ----
cost_sources_n_1col <- ggplot(sources_of_costs %>% 
...
# central ----
cost_sources_c_1col <- ggplot(sources_of_costs %>% 
...
# east ----
cost_sources_e_1col <- ggplot(sources_of_costs %>% 
...
# combine vertical ----
cost_sources_1col <- cost_sources_n_1col + cost_sources_c_1col + cost_sources_e_1col + plot_layout(ncol = 1) +
  plot_annotation(
    #tag_levels = 'a', 
    tag_levels = list(c("(a) North", "(b) Central" , "(c) East")),
    tag_prefix = '',  
...
# combine horizontal ----
cost_sources_3col <- cost_sources_n_1col + cost_sources_c_1col + cost_sources_e_1col + plot_layout(ncol = 2) +
  plot_annotation(
    #tag_levels = 'a', 
...
# combine square ----
cost_sources_2col <- cost_sources_n_1col + cost_sources_c_1col + cost_sources_e_1col + plot_layout(ncol = 2, nrow = 2) +
  plot_annotation(
    #tag_levels = 'a', 
