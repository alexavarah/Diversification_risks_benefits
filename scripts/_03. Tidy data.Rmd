...
# 5. Join datasets and calculate weighted values (current densities)
...
## 5.1. Calculate weighted values (including opportunity and productivity costs)  
...
## 5.2. Calculate cost due to lost wheat yield (original)
...
rm(econdata_low, bau_low, bau_low_tidy, cww_low, cww_low_tidy, mit_low, mit_low_tidy)
```

# 6. Join datasets and calculate weighted values (worst case data)
Calculate weighted gross profit, all other weighted variables, and opportunity and productivity costs.
```{r calculate-weighted-variables-worstcase}
wtd_worstcase <- left_join(simden_worstcase, econdata, by="ID", relationship = "many-to-many") %>% 
  # Calculate weighted gross profit for each row for the affected variables
  dplyr::mutate(
    wtd_gp = case_when(
      modelrun == "low" ~ al*grossprof,
  ...
  # Calculate opportunity cost of switching out of BAU
  group_by(imp, year, initcondit) %>% 
  dplyr::mutate(wtd_opp_cost = wtd_gp - wtd_gp[rotn_seq == "BBB"]) %>% #this is effectively MIT-BAU, so positive numbers means MIT wins
  ungroup() %>% 
  
  # Calculate weighted wheat yield and productivity cost of switching out of BAU
  dplyr::mutate(ww_yield = ifelse(stringr::str_detect(crop,'wheat'),wtd_yield,0)) %>% # for any crop that isn't wheat, assign zero yield
  group_by(imp, year, initcondit) %>%
