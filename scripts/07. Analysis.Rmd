...
library(tidyverse) # for east data manip and plotting
...
library(forcats) # for easy re-coding of factors
...
## 3.2. Summarise final weed density, gross profit, opp costs, yield & carbon across rotations 
...
### 3.2.1. Carbon, gross profit, yield (rotation mean and differences between BAU and MIT)
Here, the mean is the rotation mean and the min and max values give range across years within a rotation.  
```{r, attr.source='.numberLines'}
trdoff_rot <- c_gp_yld %>% 
  dplyr::group_by(scenario, region, soiltype, initDR) %>% 
  # calculate min and max values (this grouping will give range across years within a rotation)----
  dplyr::mutate(
...
### 3.2.2. Weed density ((a) rotation mean density and (b) proportion h & vh in year 6)
This gives mean for each level of initial density and resistance. The min and max values show the lowest and highest rotation mean % change, or year 6 mean % change, across regions and soil types.  
```{r, attr.source='.numberLines'}
density_rot <- res_tidy %>%
  dplyr::rename(initDR = initDR3) %>% # rename the column
  ...
  # Specify order of factor levels and arrange df
  dplyr::mutate( # specify factor levels
    scenario = factor(scenario, levels = c("BAU", "MIT")),
    region = factor(region, levels = c("north", "central", "east")),
    soiltype = factor(soiltype, levels = c("heavy", "medium","light")),
    initDR = factor(initDR, levels = c("LD-LR", "LD-HR", "HD-HR"))
  ...
  # Calculate mean across imputations for each year of each strategy
  group_by(scenario, initDR, region, soiltype, year) %>% 
  dplyr::mutate(
    mds_mean = mean(mds), # mean density state across imputations
    hvh_mean = mean(hvh) # mean proportion of field with high or very high density across imputations
  ) %>% 
  ungroup() %>% 
  distinct(scenario, initDR, region, soiltype, year, .keep_all = TRUE) %>% # retain only one unique row per strategy-year
  dplyr::select(scenario, initDR, region, soiltype, year, mds_mean, hvh_mean) %>% # pull out useful columns
  dplyr::rename(mds = mds_mean, hvh = hvh_mean) %>% # rename columns
  ungroup() %>% 
  dplyr::arrange(scenario, initDR, region, soiltype, year) %>% 
  
  # Calculate rotation mean density
  dplyr::group_by(scenario, initDR, region, soiltype) %>% 
  dplyr::mutate(mds_rot_mean = mean(mds)) %>% 
  ungroup() %>% 
  dplyr::filter(year == 6) %>% # retain just year 6
  
  # Calculate difference between MIT and BAU for rotation means and for yr6
  dplyr::group_by(initDR, region, soiltype) %>% 
  dplyr::mutate(
    mds_diff = mds_rot_mean - mds_rot_mean[scenario == "BAU"],
    hvhyr6_diff = hvh - hvh[scenario == "BAU"]
  ) %>% 
  ungroup() %>% 
  ...
  dplyr::mutate(
    mds_pc = (mds_diff/mds_rot_mean[scenario == "BAU"])*100, # % change in rotation mean density
    hvhyr6_pc = (hvhyr6_diff/hvh[scenario == "BAU"])*100 # % change in proportion of hvh at year 6
  ) %>% 
  ungroup() %>% 
  
  # Calculate min and max values (this grouping will give range across region and soil types)
  dplyr::filter(scenario == "MIT") %>% 
  dplyr::group_by(initDR) %>%
  dplyr::mutate(
    # min & max % change in rotation mean density
    mds_pc_min = min(mds_pc, na.rm = TRUE), 
    mds_pc_max = max(mds_pc, na.rm = TRUE), 
    # min & max % change in proportion of hvh at year 6
    hvhyr6_pc_min = min(hvhyr6_pc, na.rm = TRUE), 
    hvhyr6_pc_max = max(hvhyr6_pc, na.rm = TRUE), 
  ) %>%
  ungroup() %>%
  dplyr::select(initDR, region, soiltype, mds_pc:hvhyr6_pc_max) %>% 
  
  # Calculate mean for each level of initDR
  dplyr::group_by(initDR) %>% 
  dplyr::mutate(
    mds_pc_mean = mean(mds_pc), # mean % change in rotation mean density
    hvhyr6_pc_mean = mean(hvhyr6_pc), # mean % change in rotation mean density
  ) %>% 
  ungroup() %>% 
  
  # Tidy the data frame
  distinct(initDR, .keep_all = TRUE) %>% # retain only one unique row per initial density-resistance state
  dplyr::select (initDR, mds_pc_mean, mds_pc_min, mds_pc_max, hvhyr6_pc_mean, hvhyr6_pc_min, hvhyr6_pc_max) # retain required columns
```
...
*N.B.* There are no estimates from crop residues. In the CFT I specified 'left on field'; however, I didn't specify the amount of residue. The CFT says that a default amount will be calculated, but in fact this doesn't happen. If I wanted to re-run the analysis to include amounts for residues, I could calculate them using the equations provided in Scarlat 2010 (https://www-sciencedirect-com.ezproxy.nhm.ac.uk/science/article/pii/S0956053X10002436). Use the AHDB webside or maybe https://www.researchgate.net/publication/256793081_Straw_Use_and_Availability_for_Second_Generation_Biofuels_in_England for sugar beet or any other crops not in Scarlat 2010.

*N.B.* I didn't include harvesting field energy use. This is because all but one crop used there is harvested by combine harvester. Beet is harvested with a beet harvester. Beet harvesters and combine harvesters have similar fuel use (see https://www.mdpi.com/2077-0472/13/4/833 for combine harvester fuel use and https://www.eadt.co.uk/news/business/21765742.east-anglian-farmers-focus-fuel-efficiency/ or 'EVALUATION OF FOSSIL FUEL CONSUMPTION AND ENVIRONMENTAL IMPACTS IN SUGAR BEET PRODUCTION AS A BIOFUEL RAW MATERIAL' Fresenius Environmental Bulletin Volume 28 ± No. 4/2019 pages 2397-2406 for beet harvester fuel use). ALTHOUGH they do have different values in the Cool Farm Tool, so perhaps I should have included harvesting operations.....
...
sources <- dplyr::left_join(mean_df, min_df, by = c("region", "soiltype", "initDR", "source_of_emissions")) %>% 
  ...
  dplyr::mutate(across('source_of_emissions', ~ str_replace(., 'pesticide', 'herbicide'))) %>% 
  dplyr::mutate(across('source_of_emissions', ~ str_replace(., 'feu', 'field_energy_use'))) %>% 
  dplyr::mutate(across('source_of_emissions', ~ str_replace(., 'fert_appl', 'field_N20_from_fertiliser'))) %>% 
...
# 4. Tables
## 4.1. Mean difference in C emissions per initDR 
### 4.1.1. Calculate difference for each year, take mean across rotation
(a) WRONG This gives min and max values across regions and soil types.  
```{r}
c_gp_yld %>%
...
```{r}
c_gp_yld %>%
  filter(scenario == 'MIT', initDR == 'LD-HR') %>%
  dplyr::group_by(scenario, region, soiltype, initDR) %>%
  mutate(max_diff = max(c_emiss_diff, na.rm = TRUE)) %>%
  select(scenario, region, soiltype, initDR, max_diff) %>%
  dplyr::distinct(scenario, region, soiltype, initDR, .keep_all = TRUE)
```
The highest maximum difference for LD-HR across all regions (n=3) and soil types (n=3) is 1136.96 (this is from east, heavy).
...
c_gp_yld %>%
  filter(scenario == 'MIT', initDR == 'LD-HR') %>%
  dplyr::group_by(scenario, region, soiltype, initDR) %>%
  mutate(min_diff = min(c_emiss_diff, na.rm = TRUE)) %>%
  select(scenario, region, soiltype, initDR, min_diff) %>%
  dplyr::distinct(scenario, region, soiltype, initDR, .keep_all = TRUE)
```
The lowest maximum difference for LD-HR across all regions (n=3) and soil types (n=3) is -2790.15 (this is from east, heavy).


Emissions differ by soil type (due to different fertiliser use):
```{r}
c_gp_yld %>%
  filter(scenario == 'MIT', region == 'north', initDR == 'HD-HR') %>%
  select(scenario, region, soiltype, initDR, year, crop, c_emiss_diff) %>%
  print(n = nrow(.))
```

(b) Calculate annual mean using c_emiss_diff (which is calculated for each year separately).
```{r}
c_gp_yld %>%
  dplyr::filter(scenario == "MIT") %>% 
  
  # 1. Get rotation mean, plus annual min and max differences for each strategy
  dplyr::group_by(scenario, region, soiltype, initDR) %>% 
  dplyr::mutate(
    ghg_diff_annual_mean = mean(c_emiss_diff),
    ghg_diff_annual_min = min(c_emiss_diff), # this is the minimum difference of any year for a particular rotation
    ghg_diff_annual_max = max(c_emiss_diff)
  ) %>%
  ungroup() %>%
  select(scenario, region, soiltype, initDR, ghg_diff_annual_mean, ghg_diff_annual_min, ghg_diff_annual_max) %>%
  dplyr::distinct(scenario, region, soiltype, initDR, .keep_all = TRUE) %>% 
    
  # 2. Calculate mean, min, max for each level of initDR
  dplyr::group_by(initDR) %>% 
  dplyr::mutate(
    ghg_annualdiff_mean = mean(ghg_diff_annual_mean),
    # min and max of the rotation means
    ghg_annualdiff_min = min(ghg_diff_annual_min),
    ghg_annualdiff_max = max(ghg_diff_annual_max),
    # mean across all the min (or max) values of any year for any rotation in each level of initDR
    # in other words, the mean value across the lowest (or highest) annual differences from each rotation
    # or, the mean across the minimum differences from any year for each particular rotation within one initDR level
    ghg_annualdiff_min_mean = mean(ghg_diff_annual_min),
    ghg_annualdiff_max_mean = mean(ghg_diff_annual_max),
    # min and max difference from any rotation in each level of initDR
    overall_min = min(ghg_diff_annual_min),
    overall_max = max(ghg_diff_annual_max)
  ) %>% 
  
  # 4. Select just useful columns and filter out duplicates
  dplyr::select(initDR, 
                ghg_annualdiff_mean, ghg_annualdiff_min, ghg_annualdiff_max, 
                ghg_annualdiff_min_mean, ghg_annualdiff_max_mean,
...
### 4.1.2. Sum over rotation, calculate difference
Try doing it another way. Instead of working out the difference for each year of the rotation and then taking the mean difference, sum across years to give a rotation total and then find the difference.
```{r}
# c_gp_yld %>%
#   
#   # 1. Sum over years
...
c_gp_yld %>%
  ...
  # 1. Get total for each strategy
  dplyr::group_by(scenario, region, soiltype, initDR) %>% 
  dplyr::mutate(
    ghg_rot_tot = sum(c_total)
  ) %>%
  ungroup() %>%
  select(scenario, region, soiltype, initDR, ghg_rot_tot) %>%
  dplyr::distinct(scenario, region, soiltype, initDR, .keep_all = TRUE) %>% 
  
  # 2. Calculate difference between MIT and BAU
  dplyr::group_by(region, soiltype, initDR) %>% 
  dplyr::mutate(
    ghg_rot_tot_diff = ghg_rot_tot - ghg_rot_tot[scenario == "BAU"]
  ) %>% 
  ungroup() %>% 
    
  # 3. Calculate mean, min, max for each level of initDR
  dplyr::group_by(initDR) %>% 
  dplyr::mutate(
    ghg_rot_tot_diff_mean = mean(ghg_rot_tot_diff),
    ghg_rot_tot_diff_min = min(ghg_rot_tot_diff),
    ghg_rot_tot_diff_max = max(ghg_rot_tot_diff)
  ) %>% 
  
  # 4. Select just useful columns and filter out duplicates
  dplyr::select(initDR, 
                ghg_rot_tot_diff_mean, ghg_rot_tot_diff_min, ghg_rot_tot_diff_max) %>% 
  dplyr::distinct(initDR, .keep_all = TRUE) %>% 
  arrange(initDR)
```
...
## 4.2. Summary table in paper: Percentage change in yr 6 weed density, gp, C, calories, landuse, for each level of initDR
### 4.2.1. Calculate % change from rotation means for 3 levels of initDR
...
### 4.2.2. Add in pc change for weed density.
```{r}
pc_change_all <- pc_change_rotn %>% 
  dplyr::left_join(density_rot, by = "initDR") %>% 
  dplyr::select(-c(mds_pc_mean, mds_pc_min, mds_pc_max)) %>% 
  dplyr::relocate(hvhyr6_pc_mean:hvhyr6_pc_max, .after = initDR) %>% 
  dplyr::rename(
    bg_density_pc_mean = hvhyr6_pc_mean,
    bg_density_pc_min = hvhyr6_pc_min,
    bg_density_pc_max = hvhyr6_pc_max
  )

# Tidy up
rm(pc_change_rotn, density_rot)

pc_change_all
```

### 4.2.3. Save output as .csv
```{r}
write.csv(pc_change_all, "../output/2025-04-17_summarytable_percent_change.csv", row.names = FALSE)
```
...
p_mgmt <- strat %>%
  ...
  ggplot(aes(fill=design_element, y=scaled_value, x=initdenres)) +
    ...
    geom_point(color="lightgrey", size=1.8, width=0.1, position=position_jitterdodge(0.3)) + #, alpha=0.1
    ...
    xlab("Scenario and initial density-resistance state") +
    ylab("Scaled value") +
...
pdf("../figures/management_histogram.pdf", height = 15, width = 18)
print(p_mgmt)
dev.off()
```
...
## 5.2. Impact on weed density - violin plot
...
### 5.3.2. With region on top panels
...
a_oc_1col <- ggplot(trdoff_rot %>% dplyr::filter(scenario == "MIT"), 
  ...
  scale_colour_manual(values = c("#44AA99", "#CC6677"),
...
pdf("../figures/all_outcomes_1col.pdf", height = 30, width = 10)
print(facetpanel_1col)
dev.off()
```
...
### 5.3.3. With initial conditions on top panels
...
# Plot
## opp costs ----
a_oc_alt <- ggplot(trdoff_rot %>% 
                     dplyr::filter(scenario == "MIT") %>% 
                             dplyr::mutate(region = fct_recode(region, N = "north", C = "central", E = "east")), 
               aes(region, oc_mean, colour = oc_mean < 0)) + 
  scale_colour_manual(values = c("#44AA99", "#CC6677"),
                      labels = c("MIT wins", "MIT loses")) +
...
## yield ----
b_cal_alt <- ggplot(trdoff_rot %>% 
                     dplyr::filter(scenario == "MIT") %>% 
                             dplyr::mutate(region = fct_recode(region, N = "north", C = "central", E = "east")), 
               aes(region, calories_diff_mean, colour = calories_diff_mean < 0)) + 
  scale_colour_manual(values = c("#44AA99", "#CC6677"),
                      labels = c("MIT wins", "MIT loses")) +
  facet_grid(soiltype ~ initDR,
             labeller = labeller(soiltype = soiltype.labs, initDR = denres.labs),
...
## GHG emissions ----
c_ghg_alt <- ggplot(trdoff_rot %>% 
                     dplyr::filter(scenario == "MIT") %>% 
                             dplyr::mutate(region = fct_recode(region, N = "north", C = "central", E = "east")), 
               aes(region, c_emiss_diff_mean, colour = c_emiss_diff_mean > 0)) + 
    scale_colour_manual(values = c("#44AA99", "#CC6677"),
                      labels = c("MIT wins", "MIT loses")) +
  facet_grid(soiltype ~ initDR,
             labeller = labeller(soiltype = soiltype.labs, 
...
# Combine plots ----
facetpanel_denres_top <- a_oc_alt + b_cal_alt + c_ghg_alt + plot_layout(ncol = 1) +
  plot_annotation(tag_levels = 'a', tag_prefix = '(',  tag_suffix = ')') & 
  theme(plot.tag = element_text(size = 22, face="bold"))
```
...
pdf("../figures/all_outcomes_1col_denres-top.pdf", height = 30, width = 10)
print(facetpanel_denres_top)
dev.off()
```
...
# 6. Plots - Supplementary Info
## 6.1. Change in GHG emissions by source
...
```{r}
# north ----
sources_n_1col <- ggplot(sources %>% 
                           dplyr::filter(region == "north") %>% 
                           mutate(source_of_emissions = factor(source_of_emissions, levels = c("fertiliser_production", 
                                                                                               "field_N20_from_fertiliser", 
                                                                                               "field_energy_use",
                                                                                               "herbicide"))),
               aes(source_of_emissions, rotation_mean, colour = rotation_mean > 0)) + 
  scale_colour_manual(values = c("#009E73", "#CC6677"), labels = c("MIT wins", "MIT loses")) +
  facet_grid(soiltype ~ initDR, 
             labeller = labeller(soiltype = soiltype.labs, initDR = denres.labs)#,
             #scales="free_y"
             ) +
...
sources_c_1col <- ggplot(sources %>% 
                           ...
                           mutate(source_of_emissions = factor(source_of_emissions, levels = c("fertiliser_production", 
                                                                                               ...
                                                                                               "herbicide"))), 
               aes(source_of_emissions, rotation_mean, colour = rotation_mean > 0)) + 
  scale_colour_manual(values = c("#009E73", "#CC6677"),
                      labels = c("MIT wins", "MIT loses")) +
  facet_grid(soiltype ~ initDR,
             labeller = labeller(soiltype = soiltype.labs, 
                                 initDR = denres.labs)
...
## 6.2. Difference in profit by cost component
### 6.2.1. Calculate the differences between BAU and MIT for required variables  
  
Herbicide cost (£/ha) = ('bgherb' application rate (l/ha) × bgherb price (£/l)) + ('glyphosate' application rate (l/ha) × (glyphosate price (£/l))  
Fertiliser cost (£/ha) = (N fertiliser rate (kg/ha) × N Fertiliser price (£/kg)) + (P fertiliser rate (kg/ha) × P fertiliser price (£/kg)) + (K fertiliser rate (kg/ha) × K fertiliser price (£/kg))  
Seed cost (£/ha) = Seed rate (kg/ha) × Seed price (£/kg)  
Sundry cost (£/ha) is primarily the cost of other chemicals and other miscellaneous costs.  
Variable cost (£/ha) = Fertiliser cost + Seed cost + Herbicide cost + Sundry cost  
Fuel cost (£/ha) = Machine work rate (h/ha) × Fuel consumption (l/h) × Fuel price (£/l)  
Labour cost (£/ha) = Work rate (h/ha) × Hourly labour wage (£/h)  
Operation cost (£/ha) = Fuel cost + Labour cost  
  
Use costs for herbicide, fertiliser, operations, seed, sundry. Also costs due to yield loss.  
```{r}
#rm(cost_differences)
cost_differences <- c_gp_yld %>% 

# 1. Calculate difference between MIT and BAU
  dplyr::group_by(initDR, region, soiltype, year) %>% 
  dplyr::mutate(
    output_diff = output - output[scenario == "BAU"], 
    cyl_diff = cyl - cyl[scenario == "BAU"], 
    herb_diff = herbicide_cost - herbicide_cost[scenario == "BAU"],
    fert_diff = fertiliser_cost - fertiliser_cost[scenario == "BAU"],
    ops_diff = operations_cost - operations_cost[scenario == "BAU"],
    seed_diff = seed_cost - seed_cost[scenario == "BAU"],
    sundry_diff = sundry_cost - sundry_cost[scenario == "BAU"]
  ) %>% 
  ungroup() %>% 

# 2. Get rotation min and max
  dplyr::filter(scenario == "MIT") %>% 
  dplyr::group_by(initDR, region, soiltype) %>% # this grouping will give range across years
  dplyr::mutate(
    # min & max difference in output
    output_diff_min = min(output_diff, na.rm = TRUE), 
    output_diff_max = max(output_diff, na.rm = TRUE), 
    # min & max difference in cost due to yield losses
    cyl_diff_min = min(cyl_diff, na.rm = TRUE), 
    cyl_diff_max = max(cyl_diff, na.rm = TRUE), 
    # min & max difference in cost due to herbicides
    herb_diff_min = min(herb_diff, na.rm = TRUE), 
    herb_diff_max = max(herb_diff, na.rm = TRUE), 
    # min & max difference in cost due to fertilisers
    fert_diff_min = min(fert_diff, na.rm = TRUE), 
    fert_diff_max = max(fert_diff, na.rm = TRUE), 
    # min & max difference in cost due to fuel, labour, operations
    ops_diff_min = min(ops_diff, na.rm = TRUE), 
    ops_diff_max = max(ops_diff, na.rm = TRUE), 
    # min & max difference in cost due to fuel, labour, operations
    seed_diff_min = min(seed_diff, na.rm = TRUE), 
    seed_diff_max = max(seed_diff, na.rm = TRUE), 
    # min & max difference in cost due to sundries and variables
    sundry_diff_min = min(sundry_diff, na.rm = TRUE), 
    sundry_diff_max = max(sundry_diff, na.rm = TRUE)
  ) %>%
  ungroup() %>%

# 3. Calculate rotation mean
  dplyr::group_by(initDR, region, soiltype) %>% 
  dplyr::mutate(
    output_diff_mean = mean(output_diff),
    cyl_diff_mean = mean(cyl_diff), 
    herb_diff_mean = mean(herb_diff), 
    fert_diff_mean = mean(fert_diff), 
    ops_diff_mean = mean(ops_diff), 
    seed_diff_mean = mean(seed_diff),
    sundry_diff_mean = mean(sundry_diff)
  ) %>% 
  ungroup() %>% 
  
# 4. Retain just useful rows/columns & tidy
  distinct(initDR, region, soiltype, .keep_all = TRUE) %>%
  dplyr::select (initDR, region, soiltype, 
                 output_diff_mean, output_diff_min, output_diff_max,
                 cyl_diff_mean, cyl_diff_min, cyl_diff_max,
                 herb_diff_mean, herb_diff_min, herb_diff_max,
                 fert_diff_mean, fert_diff_min, fert_diff_max,
                 ops_diff_mean, ops_diff_min, ops_diff_max,
                 seed_diff_mean, seed_diff_min, seed_diff_max,
                 sundry_diff_mean, sundry_diff_min, sundry_diff_max
                 )
```


### 6.2.2. Convert from wide to long for plotting
```{r}
mean_cost_diff_df <- cost_differences %>%
  dplyr::select(region, soiltype, initDR, contains('mean')) %>% 
  dplyr::rename_at(vars(everything()), ~ sub("_diff_mean", "", .x)) %>% # remove '_diff_mean' from column names
  pivot_longer(
    cols = output:sundry,
    names_to = "source_of_cost_difference",
    values_to = "rotation_mean",
    values_drop_na = TRUE
  )

min_cost_diff_df <- cost_differences %>%
  dplyr::select(region, soiltype, initDR, contains('min')) %>% 
  dplyr::rename_at(vars(everything()), ~ sub("_diff_min", "", .x)) %>% # remove '_diff_min' from column names
  pivot_longer(
    cols = output:sundry,
    names_to = "source_of_cost_difference",
    values_to = "rotation_min",
...
max_cost_diff_df <- cost_differences %>%
  dplyr::select(region, soiltype, initDR, contains('max')) %>% 
  dplyr::rename_at(vars(everything()), ~ sub("_diff_max", "", .x)) %>% # remove '_diff_max' from column names
  pivot_longer(
    cols = output:sundry,
    names_to = "source_of_cost_difference",
    values_to = "rotation_max",
...
# Join data frames
sources_of_costs <- dplyr::left_join(mean_cost_diff_df, min_cost_diff_df, by = c("region", "soiltype", "initDR", "source_of_cost_difference")) %>% 
  dplyr::left_join(., max_cost_diff_df, by = c("region", "soiltype", "initDR", "source_of_cost_difference")) %>% 
  dplyr::mutate(across('source_of_cost_difference', ~ str_replace(., 'cyl', 'lost yield'))) %>%
  dplyr::mutate(across('source_of_cost_difference', ~ str_replace(., 'herb', 'herbicide'))) %>%
  dplyr::mutate(across('source_of_cost_difference', ~ str_replace(., 'ops', 'operations'))) %>%
  dplyr::mutate(across('source_of_cost_difference', ~ str_replace(., 'fert', 'fertiliser'))) %>% 
  dplyr::mutate(across('source_of_cost_difference', ~ str_replace(., 'seed', 'seed'))) %>% 
  dplyr::mutate(across('source_of_cost_difference', ~ str_replace(., 'sundry', 'sundry'))) %>% 
  dplyr::mutate(source_of_cost_difference = factor(source_of_cost_difference,
                                                   levels = c(
                                                     "output",
                                                     "lost yield", 
                                                     "herbicide",
...
# Tidy up
rm(mean_cost_diff_df, min_cost_diff_df, max_cost_diff_df)
```
...
Make plots & combine.  
A -ve value means MIT wins (green = MIT wins, orange = MIT loses) 
```{r}
# north ----
cost_sources_north <- ggplot(sources_of_costs %>% 
                           dplyr::filter(region == "north" & !grepl("output", source_of_cost_difference)),
               aes(source_of_cost_difference, rotation_mean, colour = rotation_mean > 0)) + 
  scale_colour_manual(values = c("#009E73", "#CC6677"), 
                      labels = c("MIT wins", "MIT loses")) +
  facet_grid(soiltype ~ initDR, 
             labeller = labeller(soiltype = soiltype.labs, initDR = denres.labs)) +
  scale_x_discrete(guide = guide_axis(angle = 45)) +
  labs(x = "Source of costs", 
       y = expression(atop("Mean annual difference in costs",
                           paste("between scenarios (MIT-BAU) (£ / ha)")))) +
  geom_pointrange(aes(ymin = rotation_min, ymax = rotation_max), 
                  position=position_dodge(width=0.3),
                  size = 0.9) +
  geom_hline(yintercept=0, linetype="dashed", color = "black") +
...
# central ----
cost_sources_central <- ggplot(sources_of_costs %>% 
                           dplyr::filter(region == "central" & !grepl("output", source_of_cost_difference)),
               aes(source_of_cost_difference, rotation_mean, colour = rotation_mean > 0)) + 
  scale_colour_manual(values = c("#009E73", "#CC6677"), 
                      labels = c("MIT wins", "MIT loses")) +
  facet_grid(soiltype ~ initDR,
...
# east ----
cost_sources_east <- ggplot(sources_of_costs %>% 
                           dplyr::filter(region == "east" & !grepl("output", source_of_cost_difference)),
               aes(source_of_cost_difference, rotation_mean, colour = rotation_mean > 0)) + 
  scale_colour_manual(values = c("#009E73", "#CC6677"), 
                      labels = c("MIT wins", "MIT loses")) +
  facet_grid(soiltype ~ initDR,
...
# combine vertical ----
cost_sources_1col <- cost_sources_north + cost_sources_central + cost_sources_east + plot_layout(ncol = 1) +
  plot_annotation(
    tag_levels = 'a',
    #tag_levels = list(c("(a) N", "(b) C" , "(c) E")),
    tag_prefix = '(',
    tag_suffix = ')'
    ) &
  theme(plot.tag = element_text(size = 22, face="bold"))
...
# combine horizontal ----
# cost_sources_3col <- cost_sources_north + cost_sources_central + cost_sources_east + plot_layout(ncol = 2) +
#   plot_annotation(
#     tag_levels = 'a', 
#     #tag_levels = list(c("(a) N", "(b) C" , "(c) E")),
#     tag_prefix = '(',  
...
# combine square ----
cost_sources_2col <- cost_sources_north + cost_sources_central + cost_sources_east + plot_layout(ncol = 2, nrow = 2) +
  plot_annotation(
    tag_levels = 'a', 
    #tag_levels = list(c("N", "C" , "E")),
    tag_prefix = '(',  
    tag_suffix = ')'
    ) & 
  theme(plot.tag = element_text(size = 22, face="bold"))
...
pdf("../figures/costs-only_sources_1col.pdf", height = 30, width = 10)
print(cost_sources_1col)
dev.off()
# 
# 
# pdf("../figures/costs-only_sources_3col.pdf", height = 10, width = 30)
# print(cost_sources_3col)
# dev.off()

pdf("../figures/costs-only_sources_2col.pdf", height = 25, width = 20)
print(cost_sources_2col)
dev.off()
```


## 6.3. Total costs and output
### 6.3.1. Calculate the differences between BAU and MIT for output and total costs
Add variable and operations costs to get total costs.  
  
Herbicide cost (£/ha) = ('bgherb' application rate (l/ha) × bgherb price (£/l)) + ('glyphosate' application rate (l/ha) × (glyphosate price (£/l))  
Fertiliser cost (£/ha) = (N fertiliser rate (kg/ha) × N Fertiliser price (£/kg)) + (P fertiliser rate (kg/ha) × P fertiliser price (£/kg)) + (K fertiliser rate (kg/ha) × K fertiliser price (£/kg))  
Seed cost (£/ha) = Seed rate (kg/ha) × Seed price (£/kg)  
Sundry cost (£/ha) is primarily the cost of other chemicals and other miscellaneous costs.  
TOTAL: *Variable cost (£/ha) = Fertiliser cost + Seed cost + Herbicide cost + Sundry cost*  
Fuel cost (£/ha) = Machine work rate (h/ha) × Fuel consumption (l/h) × Fuel price (£/l)  
Labour cost (£/ha) = Work rate (h/ha) × Hourly labour wage (£/h)  
TOTAL: *Operation cost (£/ha) = Fuel cost + Labour cost*  
  
```{r}
cost_output_diffs <- c_gp_yld %>% 

# 1. Sum variable and ops costs
  rowwise() %>% 
  dplyr::mutate(tot_costs = sum(variables_cost, operations_cost, na.rm = T)) %>%  # new category: h & vh
  ungroup() %>% 
...
# 2. Calculate difference between MIT and BAU
  ...
  dplyr::mutate(
    output_diff = output - output[scenario == "BAU"], 
    costs_diff = tot_costs - tot_costs[scenario == "BAU"]
  ) %>% 
  ungroup() %>% 
...
# 2. Get rotation min and max
  ...
  dplyr::mutate(
    ...
    output_diff_max = max(output_diff, na.rm = TRUE), 
    # min & max difference in cost due to yield losses
    costs_diff_min = min(costs_diff, na.rm = TRUE), 
    costs_diff_max = max(costs_diff, na.rm = TRUE)
  ) %>%
  ungroup() %>%
...
# 3. Calculate rotation mean
  ...
  dplyr::mutate(
    output_diff_mean = mean(output_diff),
    costs_diff_mean = mean(costs_diff)
  ) %>% 
  ungroup() %>% 
...
# 4. Retain just useful rows/columns & tidy
  ...
  dplyr::select (initDR, region, soiltype, 
                 output_diff_mean, output_diff_min, output_diff_max,
                 costs_diff_mean, costs_diff_min, costs_diff_max
                 )
```

### 6.3.2. Convert from wide to long for plotting
```{r}
mean_CO_diff <- cost_output_diffs %>%
  dplyr::select(region, soiltype, initDR, contains('mean')) %>% 
  dplyr::rename_at(vars(everything()), ~ sub("_diff_mean", "", .x)) %>% # remove '_diff_mean' from column names
  pivot_longer(
    cols = output:costs,
...
# Join data frames
CO_df <- dplyr::left_join(mean_CO_diff, min_CO_diff, by = c("region", "soiltype", "initDR", "source")) %>% 
  dplyr::left_join(., max_CO_diff, by = c("region", "soiltype", "initDR", "source")) %>% 
  dplyr::mutate(across('source', ~ str_replace(., 'costs', 'total costs'))) %>% 
  dplyr::mutate(source = factor(source, levels = c("output", "total costs")))
  
# Tidy up
rm(mean_CO_diff, min_CO_diff, max_CO_diff)
```
...
Plot output and costs - without the max and min as the ranges make the points so close to zero that can't see much.  
For output, a +ve value means MIT wins (green = MIT wins, pink = MIT loses)  
For costs, a -ve value means MIT wins (green = MIT wins, pink = MIT loses)  
```{r}
rm(CO_df_2)
# Create a new column to specify colours
CO_df_2 <- CO_df %>%
  dplyr::mutate(color = case_when(
    grepl("costs", source) & rotation_mean <= 0 ~ "#009E73",  # If 'costs' and y <= 0, color green
    grepl("costs", source) & rotation_mean > 0  ~ "#CC6677",    # If 'costs' and y > 0, color red
    grepl("output", source) & rotation_mean < 0 ~ "#CC6677",    # If 'output' and y < 0, color red
    grepl("output", source) & rotation_mean >= 0 ~ "#009E73", # If 'output' and y >= 0, color green
    TRUE ~ NA_character_                # Default case, if no condition is met
  ))

# Create a new column for shape based on the conditions
CO_df_2 <- CO_df_2 %>%
  dplyr::mutate(pointshape = case_when(
    source == 'output' ~ 3,  # Cross symbol
    grepl('cost', source) ~ 16, # solid circle
    TRUE ~ 1  # Default to a Circle symbol for other cases
  ))

# North
CO_north <- ggplot(CO_df_2 %>% dplyr::filter(region == "north"),
       aes(x = source, y = rotation_mean)) +
  geom_point(aes(shape = factor(pointshape), color = color, size = 3)) +  # Shape and color aesthetics
  scale_shape_manual(values = c(3, 16)) +  # Cross (3) and Circle (16)
  scale_color_identity() +  # Use the color column directly
  facet_grid(soiltype ~ initDR,
             labeller = labeller(soiltype = soiltype.labs, initDR = denres.labs)
             )  +
  labs(x = "Output and costs", 
       y = expression(atop("Mean annual difference between",
                           paste("scenarios (MIT-BAU) (£ / ha)")))
       ) +
  geom_hline(yintercept=0, linetype="dashed", color = "black") +
  theme_bw() + 
...
# Central
CO_central <- ggplot(CO_df_2 %>% dplyr::filter(region == "central"),
       aes(x = source, y = rotation_mean)) +
  geom_point(aes(shape = factor(pointshape), color = color, size = 3)) +  # Shape and color aesthetics
  scale_shape_manual(values = c(3, 16)) +  # Cross (3) and Circle (1)
  scale_color_identity() +  # Use the color column directly
  facet_grid(soiltype ~ initDR,
             labeller = labeller(soiltype = soiltype.labs, initDR = denres.labs)
             )  +
...
# East
CO_east <- ggplot(CO_df_2 %>% dplyr::filter(region == "east"),
       aes(x = source, y = rotation_mean)) +
  geom_point(aes(shape = factor(pointshape), color = color, size = 3)) +  # Shape and color aesthetics
  scale_shape_manual(values = c(3, 16)) +  # Cross (3) and Circle (1)
  scale_color_identity() +  # Use the color column directly
  ...
  scale_x_discrete(guide = guide_axis(angle = 45)) +
  labs(x = "Income (farm output) and expenditure (costs)", 
       y = expression(atop("Mean annual difference between",
                           paste("scenarios (MIT-BAU) (£ / ha)")))
       ) +
  geom_hline(yintercept=0, linetype="dashed", color = "black") +
...
# Combine
CO_1col <- CO_north + CO_central + CO_east + plot_layout(ncol = 1) +
  plot_annotation(
    tag_levels = list(c("(a) North", "(b) Central" , "(c) East")),
    tag_prefix = '',  
    tag_suffix = ''
...
pdf("../figures/costs_and_output_1col.pdf", height = 30, width = 10)
print(CO_1col)
dev.off()
```
...
## Costs and output plots
Now plot output and total costs for comparison:
A -ve value means MIT wins (green = MIT wins, orange = MIT loses) 
```{r}
# north ----
output_north <- ggplot(sources_of_costs %>% 
                           dplyr::filter(region == "north" & grepl("output", source_of_cost_difference)),
               aes(source_of_cost_difference, rotation_mean, colour = rotation_mean > 0)) + 
  scale_colour_manual(values = c("#CC6677", "#009E73"), 
                      labels = c("MIT loses", "MIT wins")) +
  facet_grid(soiltype ~ initDR, 
             labeller = labeller(soiltype = soiltype.labs, initDR = denres.labs)
             ) +
  scale_x_discrete(guide = guide_axis(angle = 45)) +
...
# central ----
output_central <- ggplot(sources_of_costs %>% 
                           dplyr::filter(region == "central" & grepl("output", source_of_cost_difference)),
               aes(source_of_cost_difference, rotation_mean, colour = rotation_mean > 0)) + 
  scale_colour_manual(values = c("#CC6677", "#009E73"), 
                      labels = c("MIT loses", "MIT wins")) +
  ...
  labs(x = "Source of costs", 
       y = expression(atop("Mean annual difference in costs",
                           paste("between scenarios (MIT-BAU) (£ / ha)")))
       #y = expression("Difference in costs between scenarios (MIT-BAU) (£ / ha)")
       ) +
  geom_pointrange(aes(ymin = rotation_min, ymax = rotation_max), 
                  position=position_dodge(width=0.3),
                  size = 0.9) +
...
# east ----
output_east <- ggplot(sources_of_costs %>% 
                           dplyr::filter(region == "east" & grepl("output", source_of_cost_difference)),
               aes(source_of_cost_difference, rotation_mean, colour = rotation_mean > 0)) + 
  scale_colour_manual(values = c("#CC6677", "#009E73"), 
                      labels = c("MIT loses", "MIT wins")) +
...
# combine
output_1col <- output_north + output_central + output_east + plot_layout(ncol = 1) +
  plot_annotation(
    #tag_levels = 'a', 
    tag_levels = list(c("(a) North", "(b) Central" , "(c) East")),
    tag_prefix = '',  
...
pdf("../figures/output_1col.pdf", height = 30, width = 10)
print(output_1col)
dev.off()
```
...
Plot again but include output this time.
For output, a +ve value means MIT wins.
For costs, a -ve value means MIT wins (green = MIT wins, orange = MIT loses) 
```{r}
# # Specify colours
# sources_of_costs <- sources_of_costs %>%
#   dplyr::mutate(color = case_when(
#     grepl("costs", source_of_cost_difference) & rotation_mean <= 0 ~ "#009E73",  # If 'costs' and y <= 0, color green
#     grepl("costs", source_of_cost_difference) & rotation_mean > 0  ~ "#CC6677",    # If 'costs' and y > 0, color red
#     grepl("output", source_of_cost_difference) & rotation_mean < 0 ~ "#CC6677",    # If 'output' and y < 0, color red
#     grepl("output", source_of_cost_difference) & rotation_mean >= 0 ~ "#009E73", # If 'output' and y >= 0, color green
#     TRUE ~ NA_character_                # Default case, if no condition is met
#   ))
# 
# 
# 
# # north ----
# costoutput_sources_n_1col <- ggplot(sources_of_costs %>% 
#                            dplyr::filter(region == "north"),
#                aes(source_of_cost_difference, rotation_mean, colour = color)) + 
...
#   scale_x_discrete(guide = guide_axis(angle = 45)) +
#   labs(x = "Source of costs", 
#        y = expression(atop("Mean annual difference in output",
#                            paste("and costs between scenarios (MIT-BAU) (£ / ha)"))) 
#        #y = expression("Difference in output and costs between scenarios (MIT-BAU) (£ / ha)")
#        ) +
#   geom_pointrange(aes(ymin = rotation_min, ymax = rotation_max), 
#                   position=position_dodge(width=0.3),
#                   size = 0.9) +
...
# 
# # central ----
# costoutput_sources_c_1col <- ggplot(sources_of_costs %>% 
#                            dplyr::filter(region == "central"),
#                aes(source_of_cost_difference, rotation_mean, colour = color)) + 
#   scale_colour_identity() +
#   facet_grid(soiltype ~ initDR,
...
# 
# # east ----
# costoutput_sources_e_1col <- ggplot(sources_of_costs %>% 
#                            dplyr::filter(region == "east"),
#                aes(source_of_cost_difference, rotation_mean, colour = color)) + 
#   scale_colour_identity() +
#   facet_grid(soiltype ~ initDR,
...
# 
# # combine
# costoutput_sources_1col <- costoutput_sources_n_1col + costoutput_sources_c_1col + costoutput_sources_e_1col + plot_layout(ncol = 1) +
#   plot_annotation(
#     #tag_levels = 'a', 
#     tag_levels = list(c("(a) North", "(b) Central" , "(c) East")),
#     tag_prefix = '',  
...
# pdf("../figures/costs-output_sources_1col.pdf", height = 30, width = 10)
# print(costoutput_sources_1col)
# dev.off()
```



Plot again, but include output mean (without the max and min) this time.
For output, a +ve value means MIT wins.
For costs, a -ve value means MIT wins (green = MIT wins, orange = MIT loses) 
```{r}
# # First, replace output min and max with NA
# # Replace values in columns B and C with NA where A is 'output'
# sources_of_costs_2 <- sources_of_costs %>%
#   dplyr::mutate(
#     rotation_min = ifelse(source_of_cost_difference == "output", NA, rotation_min),
#     rotation_max = ifelse(source_of_cost_difference == "output", NA, rotation_max)
#   )
# 
# 
# # Create a new column to specify colours
# sources_of_costs_2 <- sources_of_costs_2 %>%
#   dplyr::mutate(color = case_when(
#     grepl("costs", source_of_cost_difference) & rotation_mean <= 0 ~ "#009E73",  # If 'costs' and y <= 0, color green
#     grepl("costs", source_of_cost_difference) & rotation_mean > 0  ~ "#CC6677",    # If 'costs' and y > 0, color red
#     grepl("output", source_of_cost_difference) & rotation_mean < 0 ~ "#CC6677",    # If 'output' and y < 0, color red
...
# sources_of_costs_2 <- sources_of_costs_2 %>%
#   dplyr::mutate(pointshape = case_when(
#     source_of_cost_difference == 'output' ~ 3,  # Cross symbol
#     grepl('cost', source_of_cost_difference) ~ 16, # solid circle
#     TRUE ~ 1  # Default to a Circle symbol for other cases
#   ))
# 
# 
# 
# ggplot(sources_of_costs_2 %>% dplyr::filter(region == "north"),
#        aes(x = source_of_cost_difference, y = rotation_mean)) +
#   geom_point(aes(shape = factor(pointshape), color = color)) +  # Shape and color aesthetics
#   # geom_pointrange(aes(ymin = rotation_min, ymax = rotation_max, color = color), 
#   #                 position=position_dodge(width=0.3),
#   #                 size = 0.9) +
#   scale_shape_manual(values = c(3, 16)) +  # Cross (3) and Circle (1)
#   scale_color_identity() +  # Use the color column directly
